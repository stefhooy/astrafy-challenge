...
- **Thresholds and labels centralized in a macro + vars**, not repeated/hardcoded SQL:
  `macros/segment_from_order_count.sql` reads `returning_order_threshold` / `vip_order_threshold`
  from `dbt_project.yml`. Year boundaries (`orders_mart_years`, `segmentation_mart_year`) are
